function fitBypassAcetone
%==============================================================================
% Fit the triple bypass acetone peak curves
%==============================================================================
...
end % fitBypassAcetone

function res = objectiveFunc(params)

    % Load the triple experimental datasets
    dataset = cell(3, 1);
    dataset{1} = load('bypassAcetone1.dat');
    dataset{2} = load('bypassAcetone2.dat');
    dataset{3} = load('bypassAcetone3.dat');

    % Generate chromatographic models and invoke the simulator
    solution = createModelBypassDextran(dataset{3}(1:675, 1), params);

    % Calculate the least square residuals
    res = [];
    for i = 1:3
        res = [res; (solution(1:675, 2) - dataset{i}(1:675, 2))];
    end
    ...
    % Visualization
    figure(1); clf
    plot(dataset{1}(1:675,1), dataset{1}(1:675,2), 'b:'); hold on
    plot(dataset{2}(1:675,1), dataset{1}(1:675,2), 'b:'); hold on
    plot(dataset{3}(1:675,1), dataset{1}(1:675,2), 'b:'); hold on
    plot(solution(1:675,1), solution(1:675,2), 'r'); hold off
...
function solution = createModelBypassDextran(time, params)
    ...
    % Section 1: load
    mIn.constant(1,1)  = 99.9228; % 1% aceton solution [mol/m^3]
    % Section 2: transport
    ...
    dpfr1.interstitialVelocity  = 1; % [m/s]
    ...
    mSys = ModelSystem();
    mSys.models = [mIn, dpfr1, cstr1, cstr2, dpfr2, mOut];
