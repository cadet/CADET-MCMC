function fitLysozymeX
%==============================================================================
% Fit the lysozyme peak curves simultaneously
%==============================================================================
...
function res = objectiveFunc(params)
    ...
    enablePlot = false;
    res = [];
    weight = [0.009 0.003 0.0045 0.001 0.0015 0.0005];
    ...
    for i = 1:6
        ...
        try
            solution = createModelcolumnLysozyme(dataset(1:end, 1), params, str);
        catch e
            res = 1068000;
        ...
        res = [res; weight(i) .* (solution(1:end, 2) - dataset(1:end, 2))];

        % Visualization if enabled
        if enablePlot
            subplot(3, 2, i)
            plot(dataset(1:end,1),  dataset(1:end,2), 'b:'); hold on
            plot(solution(1:end,1), solution(1:end,2), 'r'); hold off
            legend('Meas', str);
            grid on
        end
